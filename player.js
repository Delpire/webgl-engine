...
Player.prototype.init = function(inputEngine, model){
        ...
        function _createFpsCamera(){
            ...
            fpsCamera.setSphericalCoordinates(rho, phi, theta);
            
            var a = Math.cos(phi);
			
			var xt = a * Math.sin(theta) + modelPosition[0];
			var yt = Math.sin(phi) + modelPosition[1] + height;
			var zt = a * Math.cos(theta) + modelPosition[2];
        ...
        function _createThirdPersonCamera(){
            ...
            thirdPersonCamera.setSphericalCoordinates(rho, phi, theta);
			
            var a = rho * Math.cos(phi);
            
			var x = a * Math.sin(theta) + modelPosition[0];
			var y = rho * Math.sin(phi) + modelPosition[1];
			var z = a * Math.cos(theta) + modelPosition[2];
        ...
        function _createFreeCamera(){
            ...
            freeCamera.setSphericalCoordinates(rho, phi, theta);
        }
        ...
        function _setFpsCamera(){
            ...
            if(!isNaN(mouseDelta.dx) && Math.abs(mouseDelta.dx) > 0.8){
                theta = theta - rotationSpeed * mouseDelta.dx;
                theta = _clampTheta(theta);
            }
            ...
            var a = Math.cos(phi);
            var xd = a * Math.sin(theta);
            var yd = a * Math.sin(phi);
            var zd = a * Math.cos(theta);
            ...
            var target = vec3.fromValues(a * xd + modelPosition[0], a * yd + modelPosition[1] + height, a * zd + modelPosition[2]);    
            var position = vec3.fromValues(modelPosition[0], modelPosition[1] + height, modelPosition[2]);
		
            // Update the camera's target, position, up vector, and spherical coordinates.
            fpsCamera.setCameras(target, position, upVector);
            fpsCamera.setSphericalCoordinates(1.0, phi, theta);
        }
		
        function _setThirdPersonCamera(){
            ...
            // This vector represents the direction the camera is facing.
            var a = Math.cos(phi);
            var xd = Math.sin(theta);
            var yd = Math.sin(phi);
            var zd = Math.cos(theta);
            
            model.setTransform(modelPosition[0], modelPosition[1], modelPosition[2],
                    modelRotation[0], theta, modelRotation[2],
